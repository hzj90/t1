...
;(function() {
    let toast = null;
    window.autoClick = function(matchId, marketId, pl, line) {
        let time = 0;
        const loopQuery = function() {
            time ++;
            if(time > 5) {
            ...
            const id = (marketId+'_'+line+'_'+pl).replace(/\./g, '').replace(/\+/g, '').replace(/-/g, '').replace(/ /g, '');
            const node = document.querySelector('#visualizationPanel #box'+id);
            console.log(id)
            console.log('time:',time,node);
        ...
        const midnode = document.querySelector('#mid'+matchId);
        if(midnode) {
            midnode.click();
            loopQuery();
        } else {
            if(!toast) {
                const node = `<div style="position: fixed;border-radius:3px;padding:5px 10px;background: rgba(0,0,0,1);color:#fff;
                width:76px;height:30px;top:0;bottom:0;left:0;right:0;margin:auto;z-index: 9999; ">重新选择</div>`;
                toast = $(node);
                $('body').append(toast);
            }
            $(toast).show();
